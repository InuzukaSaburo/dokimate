...
Game_MapGenerator.prototype.setEvent = function(event, targetSymbols, targetArea) {
    ...
    if (canSet) {
        ...
        if(event.enemyType) {
            $gameSystem.enemies[event._eventId] = this.generateMonster(event.enemyType);
            console.log($gameSystem.enemies[event._eventId].hp);
        }
...
Game_MapGenerator.prototype.generateMonster = function(type){
    var monster = {};
    monster.hp = 5;
    monster.atk = 4;
    monster.def = 5;
    monster.probability = 15 / 100;
    monster.exp = 5;
    switch(type) {
    case 'slime':
        break;
...
Game_MapGenerator.prototype.setStart = function() {
    ...
    if(!this._goalStart) {
        this.setPlayer(this._startXY.x, this._startXY.y);
    }
};
...
Game_MapGenerator.prototype.setGoal = function() {
    ...
    if(this._goalStart) {
        this.setPlayer(this._goalXY.x, this._goalXY.y);
    }
};

//プレイヤーの設置
Game_MapGenerator.prototype.setPlayer = function(x, y) {
...
Game_MapGenerator.prototype.setRateEvents = function() {
    ...
    mapDataRateMapEvents.forEach(function(mapDataEvent) {
        if (this.randBool(parseFloat(mapDataEvent.meta.RateMap))) {
            ...
            if(mapDataEvent.meta.EnemyType){
                event.enemyType = mapDataEvent.meta.EnemyType;
            }
            this.setEvent(event);
        }
    }, this);
...
// マップクラスのマップ自動生成
Game_Map.prototype.generateMap = function(mapType, goalStart) {
    if($gameSystem.enemies) {
        $gameSystem.enemies = null;
    }
    $gameSystem.enemies = [];
    ...
    case 'RoomAndPass':
        ...
        if(!!goalStart) {
            this._mapGenerator._goalStart = true;
        }
        break;
    case 'FillRoom':
...
Game_Interpreter.prototype.pluginCommand = function(command, args) {
    ...
    if (command === 'MapGenerator') {
        $gameMap.generateMap(args[0], args[1]);
    }
};
